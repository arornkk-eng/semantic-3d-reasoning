...
本项目构建了一套本地运行的智能三维重建与语义编辑系统。系统接收多视角图片或视频，通过 ZipSplat 前馈模型生成 3D Gaussian Splatting 场景，并在浏览器内提供实时查看、语义实例分割、二维掩码到三维 Gaussian 的投射、几何补全、独立语义图层、图层持久化和场景关系分析。
...
自动识别采用 torchvision 的 Mask R-CNN ResNet50 FPN V2。系统使用 COCO 预训练权重，对每个视角输出类别、置信度、边界框和实例 mask。

当前固定识别 19 类室内常见对象：杯子、椅子、瓶子、床、沙发、电视、笔记本电脑、键盘、鼠标、手机、书、盆栽、花瓶、时钟、冰箱、微波炉、烤箱、水槽、马桶。桌子和 desk 按产品要求排除。

当前检测阈值为 0.40。多视角结果按类别和几何重投影重叠度执行一对一实例匹配。三视角模式下，实例至少需要两个视角支持。深度有效覆盖率低于 0.15 的候选会被过滤。

### 4.3 交互式修正预留能力

项目代码已接入 SAM 2.1 Hiera Tiny 点提示分割。正点表示目标内部，负点表示需要排除的区域；后端支持单个 GPU 驻留分割会话，默认会话有效期为 10 分钟。

该能力目前属于技术预留，尚未在当前编辑器界面形成清晰、可发现、经过完整验证的用户入口，因此不计入现阶段已交付功能。当前正式用户流程以自动语义识别、实例选择、三维投射和三维补全为主。后续若启用该能力，需要补充明确的“手动修正”模式入口、正负点状态提示、清空与撤销操作、操作说明和端到端浏览器测试。
...
4. 将 Gaussian 相机深度与捕获深度图比较。
5. 检查 Gaussian 透明度和有效状态。
6. 综合多个视角的可见正证据，生成 Gaussian 索引集合。

深度门控用于区分目标表面、前景漂浮点和被遮挡点。遮挡视图被视为未知证据，不直接作为反对票，从而降低破损模型和局部遮挡带来的误删。

### 5.2 GPU 实现

编辑器新增语义 mask 与深度相交着色器。mask 以红通道采样，深度使用 R32F 纹理。GPU 为每个 Gaussian 输出状态位，包括 mask 命中、深度有效、可见、前置不一致、后置不一致和低透明度。
...
精细补全借鉴 Open3D 常见点云区域生长思想，但当前没有直接依赖 Open3D，而是使用 NumPy、SciPy `cKDTree` 和局部 PCA 实现：
...
浏览器只缓存未提交编辑状态。持久语义图层位于 `data/layers/{task_id}/{layer_id}`。删除图层前显示影响确认，并级联处理引用该图层的场景快照；原始 PLY 不受影响。

## 八、场景理解与快照

用户点击“理解”后，系统只分析当前选中的已保存语义图层，并以点击时的当前相机为参考系。对象位置使用 Gaussian 中心的中位数和 5% 至 95% 分位边界计算，以降低离群点影响。

当前输出高可信空间关系：左侧、右侧、上方、下方、前方、后方、近邻和二维重叠。关系采用用户视角表达，例如“瓶子1位于杯子1的左前方”。系统不显示具体距离和置信度。
...
| SAM 2.1 | 已接入的点提示分割预留能力，当前界面尚未正式呈现 | Apache 2.0；模型权重条款需单独核对 |
...
1. 后端完整测试曾达到 50 项通过；新增类别后的分割定向测试为 26 项通过。
...
1. 自动识别依赖 COCO 固定类别，无法识别训练类别之外的专业物体。
2. 新增类别是开放白名单，不等于模型对每类都具有相同精度；床、沙发等大物体在破损重建视图中仍可能漏检。
...
用 Grounding DINO 或开放词汇检测器根据文本寻找目标，再用 SAM 2 生成精细 mask。这样可减少对 COCO 固定类别的依赖，并允许用户输入“床头柜”“台灯”等自定义类别。
...
增加任务取消、GPU 队列监控、模型懒加载缓存、错误恢复、版本迁移、性能基准、第三方许可证清单和可复现部署配置。

## 十五、结论

本项目已经完成从少视图图片到 3D Gaussian 场景、从二维实例 mask 到三维对象图层、再到视角相关空间关系分析的技术闭环。系统的关键价值在于把快速 3DGS 重建结果转化为可选择、可编辑、可保存和可理解的对象级数据。

现阶段最重要的工作不是继续堆叠界面功能，而是建立标注数据和量化指标，验证新增类别、多视角投射及几何补全在不同场景中的稳定性。在此基础上再建设开放词汇识别和场景问答，技术风险更可控。
