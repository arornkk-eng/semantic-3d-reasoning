...
export default function ResultPanel({ taskId, output }: Props) {
  ...
  const viewerUrl = `/splat-viewer/index.html?content=${encodeURIComponent(fullPlyUrl)}&hpr=1&webgl&task_id=${encodeURIComponent(taskId)}`;
  const editorUrl = `/splat-editor/index.html?load=${encodeURIComponent(fullPlyUrl)}&filename=scene.ply&task_id=${encodeURIComponent(taskId)}`;
