...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    // Commit an already-fused Gaussian mask to one explicit Splat. This is the
    // integration point for asynchronous semantic/depth processing: it snapshots
    // and validates caller-owned buffers, then uses the normal undoable edit path.
    events.function('select.commit', (
        splat: Splat,
        op: 'add' | 'remove' | 'set' | 'intersect',
        selection: Uint8Array | Uint32Array
    ) => {
        const sceneSplats = scene.getElementsByType(ElementType.splat) as Splat[];
        if (!sceneSplats.includes(splat)) {
            throw new Error('Selection target is not part of the current scene');
        }
        if (op !== 'add' && op !== 'remove' && op !== 'set' && op !== 'intersect') {
            throw new Error(`Unknown selection operation: ${op}`);
        }

        const total = splat.splatData.numSplats;
        let committed: Uint8Array | Uint32Array;

        if (selection instanceof Uint8Array) {
            if (selection.length !== total) {
                throw new Error(`Selection mask length ${selection.length} does not match ${total} Gaussians`);
            }
            // Fused masks commonly contain support counts rather than 255. The
            // SelectOp byte-mask contract is exactly 255 = hit, so normalize a
            // private snapshot without retaining or mutating caller memory.
            committed = new Uint8Array(total);
            for (let i = 0; i < total; ++i) {
                committed[i] = selection[i] === 0 ? 0 : 255;
            }
        } else if (selection instanceof Uint32Array) {
            // SelectOp's cursor predicate requires sorted unique IDs. Filter the
            // background sentinel and stale/out-of-range IDs while snapshotting.
            const sorted = selection.slice().sort();
            let uniqueCount = 0;
            let previous = 0;
            let havePrevious = false;
            for (let i = 0; i < sorted.length; ++i) {
                const id = sorted[i];
                if (id < total && (!havePrevious || id !== previous)) {
                    uniqueCount++;
                    previous = id;
                    havePrevious = true;
                }
            }
            const ids = new Uint32Array(uniqueCount);
            let dst = 0;
            havePrevious = false;
            for (let i = 0; i < sorted.length; ++i) {
                const id = sorted[i];
            ...
            throw new Error('Selection must be a Uint8Array mask or Uint32Array of IDs');
        }

        const editOp = new SelectOp(splat, op, committed);
        return editHistory.add(editOp).then(() => editOp);
    });
    ...
    const performSelectionFunc = async (
        ...
        if (splats.length !== 1 || splats[0].numSelected === 0) {
            throw new Error('Exactly one Splat with selected Gaussians is required');
        }
    ...
    // Promise-returning variant used by asynchronous semantic projection. The
