...
class Picker {
    ...
    // Render all visible splats into the depth-estimation color target.
    prepareSceneDepth() {
        if (!this.depthRenderTarget) return;
        const { scene } = this;
        const emptyMap = new Map();
        this.device.scope.resolve('pickOp').setValue(3);
        this.device.scope.resolve('pickMode').setValue(1);
        this.renderPass.blendState = this.depthBlendState;
        this.renderPass.init(this.depthRenderTarget);
        this.renderPass.setClearColor(depthClearColor);
        this.renderPass.update(scene.camera.camera, scene.app.scene, [scene.splatLayer], emptyMap, false);
        this.renderPass.render();
    }
